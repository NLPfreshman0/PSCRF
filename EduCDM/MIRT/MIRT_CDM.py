...
def irt2pl(theta, a, b, *, F=np):
    ...
    #return torch.sigmoid(torch.squeeze(((theta) * a).sum(dim=1, keepdim=True), dim=1) + b)


class MIRTNet(nn.Module):
    def __init__(self, user_num, item_num, latent_dim, a_range, irf_kwargs=None, sensitive_name=None, dataset_index=None, mode=None):
        ...
        self.mlp_combine = nn.Sequential(
            nn.Linear(2*latent_dim, self.hidden_size),
    ...
    def forward(self, user, item, sensitive, labels, cls_labels, train_mode=True):
        ...
        if not train_mode:
            if self.mode == 'ours':
                ...
                return self.irf(theta, a, b, **self.irf_kwargs), binary_outputs
        ...
        if self.sensitive_name == 'escs':
            theta_eo_loss = self.calc_eo1(sensitive, self.irf(debias_theta, a, b, **self.irf_kwargs))
            Ud_eo_loss = self.calc_eo1(sensitive, self.irf(Ud, a, b, **self.irf_kwargs))
        else:
...
class MIRT(CDM):
    def __init__(self, user_num, item_num, latent_dim, a_range=None, save_path=None, sensitive_name=None, dataset_index=None, mode=None):
        ...
        self.irt_net = MIRTNet(user_num, item_num, latent_dim, a_range, sensitive_name=sensitive_name, dataset_index=dataset_index, mode=mode)
