...
class Net(nn.Module):
    ...
    def __init__(self, exer_n, student_n, knowledge_n, sensitive_name=None, index=None, mode=None):
        ...
        self.alpha = nn.Embedding(self.student_n, 1)
        self.beta = nn.Embedding(self.student_n, 1)
        self.mode = mode
        
        self.mlp_combine = nn.Sequential(
            nn.Linear(2*self.emb_dim, self.hidden_size),
            nn.BatchNorm1d(self.hidden_size),
            nn.ReLU(),
            nn.Linear(self.hidden_size, self.hidden_size//2),
            nn.BatchNorm1d(self.hidden_size//2),
            nn.ReLU(),
            nn.Linear(self.hidden_size//2, self.emb_dim),
    ...
    def forward(self, user, item, sensitive, labels, cls_labels, input_knowledge_point, train_mode=True):
        ...
        if not train_mode:
            if self.mode == 'ours':
                ...
                return self.irf(self.theta_knowledge(theta), a, b, kw), binary_outputs, self.theta_knowledge(theta), kw
        ...
        if self.sensitive_name == 'escs':
            theta_eo_loss = self.calc_eo1(sensitive, self.irf(self.theta_knowledge(debias_theta), a, b, kw))
            Ud_eo_loss = self.calc_eo1(sensitive, self.irf(self.theta_knowledge(Ud), a, b, kw))
        else:
            theta_eo_loss = self.calc_eo2(sensitive, self.irf(self.theta_knowledge(debias_theta), a, b, kw))
            Ud_eo_loss = self.calc_eo2(sensitive, self.irf(self.theta_knowledge(Ud), a, b, kw))
    ...
    def theta_knowledge(self, theta):
        batch, dim = theta.size()
        stu_emb = theta.view(batch, 1, dim).repeat(1, self.knowledge_n, 1)
        knowledge_emb = self.knowledge_emb.repeat(batch, 1).view(batch, self.knowledge_n, -1)
        return torch.sigmoid(self.stat_full(stu_emb * knowledge_emb)).view(batch, -1)
...
class KaNCD(CDM):
    def __init__(self, knowledge_n, exer_n, student_n, save_path=None, sensitive_name=None, dataset_index=None, mode=None):
        ...
        self.irt_net = Net(exer_n, student_n, knowledge_n, sensitive_name=sensitive_name, index=dataset_index, mode=mode)
        self.sensitive_name = sensitive_name
